...
uint8_t identify() {
    ...
    while(status & STAT_BSY) {
        uint32_t i = 0;
        while(1) {
            printk("Printing stuff %d\n", i);
            i++;
        }
        for(i = 0; i < 0x0FFFFFFF; i++) {}
        printk("Checking regstat.\n");
        status = inb(ATA_PRIMARY_COMM_REGSTAT);
    }
    
    if(status == 0) return 0;

    printk("Status indicates presence of a drive. Polling while STAT_BSY... ");
    while(status & STAT_BSY) {
      printk("\ninb(ATA_PRIMARY_COMM_REGSTAT);... ");
      status = inb(ATA_PRIMARY_COMM_REGSTAT);
    }
    ...
    printk("Reading IDENTIFY structure.\n");
    //uint8_t *buff = kmalloc(40960, 0, NULL);
    uint8_t buff[256 * 2];
    insm(ATA_PRIMARY_DATA, buff, 256);
    printk("Success. Disk is ready to go.\n");
    // We read it!
    return 1;
}
...
void ata_pio_read28(uint32_t LBA, uint8_t sectorcount, uint8_t *target) {
    ...
    for(i = 0; i < sectorcount; i++) {
        ...
        // Transfer the data!
        insm(ATA_PRIMARY_DATA, (void *)target, 256);
        target += 256;
    }
    ...
    for(i = 0; i < sectorcount; i++) {
        ...
        while(1) {
            ...
            if(status & STAT_DRQ) {
                ...
                printk("DISK SET ERROR STATUS!");
                halt();
            }
        }
        // Transfer the data!
        outsm(ATA_PRIMARY_DATA, (void *)target, 256);
        target += 256;
    }
